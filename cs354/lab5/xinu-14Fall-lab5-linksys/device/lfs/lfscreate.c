	...
	)
{
	...
	dir.lfd_ifree = 1;	/* Reserves first iblock for the root dir	*/
	dir.lfd_ilist = 0;
	dir.lfd_size = 0;
	retval = write(disk,(char *)&dir, 0);
	if (retval == SYSERR) {
		return SYSERR;
	}
	...
	lfibclear(&iblock, 0);
	/*Reserving the first i-block to root*/
	iblock.ib_next = LF_INULL;
	lfibput(disk, 0, &iblock);
	
	/*Putting the rest in the free list*/
