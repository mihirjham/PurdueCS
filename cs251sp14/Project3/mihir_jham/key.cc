...
void Key::keySub(Key b)
{
	...
	{
		t = digit[i];
		digit[i] = (t - b.digit[i] ) % R;
		
		if(digit[i] < 0)
		{
			digit[i-1] = digit[i-1]/2;
			digit[i] =  (digit[i] % R + R) % R;
		}
	}
}
...
bool Key::checkIndex(Key key, int index)
{
	if(digit[index] == key.digit[index])
		return true;
	else
		return false;
}
