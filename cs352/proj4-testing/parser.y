...
%token VAR EQUAL SEMICOLON NEWLINE DOC WRITE OPENPAR CLOSEPAR COMMA DOT START END OPENBRACE CLOSEBRACE COLON IF ELSE FUNCTION
...
loop:
	...
	| DO OPENBRACE NEWLINE {linenumber++;} CLOSEBRACE NEWLINE{linenumber++;} WHILE OPENPAR not_expression CLOSEPAR separator
	{
		...
		if(fromNL)
			$$ = makeNode(DoWhile, $10, NULL, NULL, linenumber-1);
		else
			$$ = makeNode(DoWhile, $10, NULL, NULL, linenumber);
